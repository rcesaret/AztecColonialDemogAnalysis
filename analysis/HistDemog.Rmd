...
x = MT1805_B_long %>% #filter(Race == "Indio") %>% 
  mutate(Pct_TotalExempt = (Proximos + NinosNinas + Governadores + Caciques + Reservados + ViudasSolteras + Ausentes)/TribClass_TotalPersons,
         Tribs = (Casados + Casadas + ViudosSolteros)/2,
         PT_ratio = TribClass_TotalPersons/Tribs)
...
x %>% select(TribClass, Pct_TotalExempt, Pct_TotalExempt_C16, PT_ratio, PT_ratio_C16) %>% 
  group_by(TribClass) %>% 
  report() %>%
  summary()
...
Juris1800_Data = Juris1800 %>% left_join(TribTimeSeries, by = "OBJECTID")
...
st_write(Partidos1800_Data, paste0(wd$data_r, "Partidos1800_Data_v3.gpkg"), driver = "GPKG", overwrite=TRUE, append=FALSE)
